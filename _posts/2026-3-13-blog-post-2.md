...
In natural language, if we have one target token embedding$$w_p$$, we can randomly sample a token in its neighborhood to be the positive sample$$w_{pos}$$, and $$N$$ tokens out of its neighborhood to be the negative samples$$w_{neg}$$, then we maximize the differences between the positive sample and the negative samples. $$\mathcal{L}=-\log\sigma(w_p^Tw_{pos})-\sum_{i=1}^{N}\log(\sigma(-w_p^Tw_{neg_i}))$$
