...
$$
\mathcal{L}(\theta) = \left\\mid u_t^\theta(x)-u_t^{target}(x)\right\\mid ^2
$$
...
\begin{align}
  ...
  &\Leftrightarrow u_t^{target}(x_t\mid z) = \dot {\mu_t(z)}-\frac{\dot {\sigma_t}}{\sigma_t}\mu_t(z)+ \frac{\dot{\sigma_t}}{\sigma_t}x_t\\
